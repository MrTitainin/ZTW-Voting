<template>
    <div class="shadow overflow-hidden sm:rounded-md">
        <div class="px-4 py-5 bg-white sm:p-6  bg-emerald-700">
            <div class="flex gap-x-2 gap-y-10 items-center justify-end" v-for="election in elections" :key="election.id">
                <div class="mt-5 col-start-auto text-emerald-200 align-middle text-right mr-1">{{ election.name }}</div>
                <button class="mt-5 flex-none inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-emerald-50 bg-emerald-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-slate-600 disabled:text-slate-200" @click="vote(election)" :disabled="!election.votable || !this.voter">Vote</button>
